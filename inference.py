...
# === Use the LiteLLM proxy provided by Scaler/Meta Hackathon ===
client = OpenAI(
...
def run_inference():
    ...
    # Make a real API call through their proxy (required for LLM Criteria Check)
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "user", "content": "Classify this email: 'I cannot login to the company dashboard. Getting invalid credentials error.'"}
            ],
            max_tokens=30,
            temperature=0.7
        )
        ...
        print(f"[STEP] step=1 action=classify reward=0.0 error=proxy_issue", flush=True)
